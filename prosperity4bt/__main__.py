...
def parse_algorithm(algorithm: Path) -> Any:
    # Add the algorithm's own directory first (so relative imports work),
    # then the working directory (so `from datamodel import ...` resolves
    # to datamodel.py at the repo root when running from there).
...
def cli(
    ...
    days: Annotated[list[str], Argument(help="Days to backtest. Use <round>-<day> for a single day (e.g. 0--1), or <round> for all days in a round (e.g. 0).", show_default=False)],
    ...
    data: Annotated[Optional[Path], Option(help="Path to a custom data directory. Must mirror the structure of prosperity4bt/resources (round0/, round1/, ...).", show_default=False, exists=True, file_okay=False, dir_okay=True, resolve_path=True)] = None,
